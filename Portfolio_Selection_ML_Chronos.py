...
for sym in symbols:
    ...
    X_train = X.iloc[:len(X)-size_test]
    X_test = X.iloc[-size_test:]
    y_reg = retornos_periodo.shift(-size_test).loc[X_train.index]
    y_bin = np.where(y_reg > 0 , 1, -1)
    y_df = y_reg.copy()
    y_train = pd.DataFrame(y_bin, index = y_reg.index).iloc[:,0]
...
winners_ret =  winners_ret.iloc[-len(y_train):]  
